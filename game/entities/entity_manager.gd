...
# Procedural spawning settings - MODIFIED FOR FAST TRAINING
@export var procedural_spawning_enabled: bool = true
@export var spawn_chance_per_chunk: float = 0.90 # 90% chance per surface chunk (increased for fast training)
@export var min_spawn_distance_from_player: float = 30.0 # Spawn closer (reduced from 40)
@export var max_spawns_per_chunk: int = 6 # More zombies per chunk (increased from 3)
